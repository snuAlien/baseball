...
#시각화2
#데이터
schools = ['휘문고', '서울고', '장충고', '덕수고', '경남고', '광주일고', '제물포고', '경북고', '경기고', '성남고']
...
ax1.set_title("고졸", fontproperties=font_prop)
ax1.set_xlim(60, 0)
ax1.set_yticks([])
...
ax2.set_title("대졸", fontproperties=font_prop)
ax2.set_xlim(0, 60)
ax2.set_yticks([])
...
# 그래프 생성
fig, ax = plt.subplots()
bars1 = ax.bar(x - width/2, pitchers, width, label='투수', color='#37bceb')
bars2 = ax.bar(x + width/2, batters, width, label='야수', color='#08043c')
...
<div style='
    ...
    margin-top: 24px;
'>
그래프를 통해 알 수 있듯이 상위 라운드에는 확실히 투수들이 선호된다는 것을 확인할 수 있다. 5년간 상위 라운드에서 계약한 투수들은 169명, 타자들은 81명으로 두 배 이상의 차이를 보인다. 전체 드래프트에서 뽑히는 투수의 수가 타자보다 많다는 점을 감안하더라도, 오히려 하위 라운드에서는 타자의 비율이 더 높다는 점을 미루어보았을 때 투수 자원은 보다 상위 라운드에서 뽑는 경향이 있다는 것을 확인할 수 있다.
            
또한 이 기간 동안 뽑힌 1차 지명자의 비율을 확인해보면 이러한 경향은 더욱 두드러진다. 사실상 해당 연도 드래프트의 주인공이라고도 불리는 1차 지명자들은, 각 구단이 최우선으로 계약하고자 하는 이른바 특급 자원들이다. 하지만 조사한 기간 동안 투수 1차 지명자는 41명, 타자 1차 지명자는 9명으로 무려 4배 이상의 엄청난 차이를 보였다. 확실히 타자보다는 투수에 우선 순위를 두고 드래프트를 진행하는 것을 확인할 수 있다.
...
data = {
    ...
    '1억원 당 WAR': [2.22, 2.05, 1.94, 1.54, 1.1, 1.34, 1.11, 1.12, 0.96, 0.89]
}
...
ax.barh(df['팀'], df['1억원 당 WAR'], color='#37bceb')
ax.set_xlabel('1억원 당 WAR',fontproperties=font_prop)
ax.set_title('KBO 팀별 드래프트 투자 효율 (통산 WAR / 억 원)',fontproperties=font_prop)
#ax.set_yticklabels(df['팀'])
